...
Generate 8 tutorial exercises based on the lecture notes above. For each exercise:
...
// Enhance pasted content with more details and examples (for manual courses)
async function enhanceContent(topicTitle, courseTitle, pastedContent, provider = 'openai', model = null, apiKey = null) {
  const prompt = `You are an expert educator. A student has pasted content from another learning platform about the following topic. Your task is to enhance this content by:

1. Explaining concepts in more detail
2. Adding more examples and practical applications
3. Clarifying any unclear points
4. Providing additional context and background information
5. Making it more comprehensive and educational

Course: ${courseTitle}
Topic: ${topicTitle}

Original Content (from other platform):
${pastedContent}

Please enhance this content by:
- Expanding on key concepts with detailed explanations
- Adding multiple examples for each concept
- Providing step-by-step explanations where applicable
- Including relevant context and background information
- Making it more comprehensive while maintaining clarity
- Formatting in clear, well-structured markdown
...
// Generate exactly 10 tutorial exercises (for manual courses)
async function generateTutorialExercisesManual(topicTitle, courseContext, lectureNotes = '', provider = 'openai', model = null, apiKey = null) {
  const prompt = `You are an expert educator. Create EXACTLY 10 tutorial exercises with answers for the following topic.

Course Context: ${courseContext}
Topic: ${topicTitle}
${lectureNotes ? `\nLecture Notes:\n${lectureNotes}\n` : ''}

Generate EXACTLY 10 tutorial exercises based on the lecture notes above. For each exercise:
- Provide a clear question or problem that relates to the concepts in the lecture notes
- Include a detailed answer/explanation in a very organized and detailed format that a beginner can understand and follow (Combination of Paragraph and points with sub-points)
...
IMPORTANT: Generate EXACTLY 10 exercises, not 8, not 12, EXACTLY 10.
Return only valid JSON, no markdown code blocks.`;
...
// Generate exactly 15 MCQ questions (for manual courses)
async function generateMCQManual(topicTitle, courseContext, lectureNotes = '', provider = 'openai', model = null, apiKey = null) {
  const prompt = `You are an expert educator. Create EXACTLY 15 Multiple Choice Questions (MCQ) for the following topic.

Course Context: ${courseContext}
Topic: ${topicTitle}
${lectureNotes ? `\nLecture Notes:\n${lectureNotes}\n` : ''}

Generate EXACTLY 15 MCQ questions based on the lecture notes above. For each question:
- Create a clear, well-formulated question
...
IMPORTANT: 
- Generate EXACTLY 15 MCQ questions, not 10, not 20, EXACTLY 15
- Each question must have exactly 4 options
- Questions should cover different aspects of the lecture notes
- Make questions progressively challenging but appropriate for the content level
...
Return only valid JSON, no markdown code blocks.`;
  ...
  try {
    const parsed = JSON.parse(jsonStr);
    return {
      mcq_questions: parsed.mcq_questions || [],
      short_answer_questions: [], // Empty for manual courses
    };
  } catch (error) {
    console.error('Failed to parse MCQ:', error);
    return {
      mcq_questions: [],
      short_answer_questions: [],
    };
